...
class ScreenRuntimeTests(unittest.TestCase):
    ...
    def test_clean_viewer_and_landing_have_no_legacy_novnc_pages(self):
        ...
        self.assertIn('data-icon="keyboard"', viewer)
        self.assertIn('class="keyboard-icon"', viewer)
        self.assertIn('id="mobile-keyboard-input"', viewer)
        self.assertIn('class="remote-keyboard-capture"', viewer)
        self.assertNotIn('id="mobile-keyboard-panel"', viewer)
        self.assertNotIn('id="mobile-keyboard-done"', viewer)
        self.assertIn('#mobile-keyboard-input{position:fixed;left:50%;bottom:0;width:1px;height:1px;opacity:.01', viewer)
        self.assertIn('keyboardInput.blur();', viewer)
        self.assertIn('document.activeElement === keyboardInput', viewer)
        ...
        self.assertIn('#mobile-keyboard-button,#mobile-help-button,#mobile-pointer-button,#mobile-clipboard-button{display:grid;place-items:center;width:48px;height:44px;min-width:48px;min-height:44px', viewer)
        self.assertIn('background:#111827e8;color:#f8fafc;box-shadow:0 8px 24px #0008', viewer)
        self.assertIn('#mobile-keyboard-button svg,#mobile-pointer-button svg,#mobile-clipboard-button svg{display:block;width:20px;height:20px}', viewer)
        self.assertIn('#mobile-keyboard #mobile-keyboard-button{position:relative;padding:0;appearance:none', viewer)
        self.assertIn('#mobile-keyboard-button .keyboard-icon{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%)}', viewer)
        self.assertIn('const KEYBOARD_SENTINEL = "\\u200b";', viewer)
        self.assertIn('function resetKeyboardInput()', viewer)
        self.assertIn('data-guide-icon="direct-touch"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="m5 3 14 9-7 1-4 7z"/><path d="m13 13 4 6"/></svg>', viewer)
        ...
        self.assertIn('aria-label="Open phone clipboard"', viewer)
        self.assertIn('id="mobile-clipboard-menu"', viewer)
        self.assertIn('id="paste-from-phone"', viewer)
        self.assertIn('id="copy-to-phone"', viewer)
        self.assertIn('Paste from phone', viewer)
        self.assertIn('Copy to phone', viewer)
        self.assertIn('addEventListener("clipboard"', viewer)
        self.assertIn('remoteClipboardText = event.detail.text || ""', viewer)
        self.assertIn('const REMOTE_CLIPBOARD_TIMEOUT_MS = 2500;', viewer)
        self.assertIn('const remoteClipboardWaiters = new Set();', viewer)
        self.assertIn('function waitForRemoteClipboardText()', viewer)
        self.assertIn('new ClipboardItem({', viewer)
        self.assertIn('"text/plain": textPromise.then(text => new Blob([text]', viewer)
        self.assertIn('navigator.clipboard.write([clipboardItem])', viewer)
        self.assertIn('navigator.clipboard.writeText(text)', viewer)
        self.assertNotIn('setTimeout(resolve, 180)', viewer)
        self.assertIn('sendRemoteCopyShortcut()', viewer)
        self.assertIn('function dismissNativeKeyboardForClipboard()', viewer)
        self.assertIn('clipboardButton.addEventListener("pointerdown", dismissNativeKeyboardForClipboard)', viewer)
        self.assertIn('window.getSelection()?.removeAllRanges();', viewer)
        self.assertNotIn('id="mobile-clipboard-fallback"', viewer)
        self.assertNotIn('id="mobile-clipboard-input"', viewer)
        self.assertNotIn('Paste from your phone', viewer)
        self.assertIn('showKeyboardPasteFallback()', viewer)
        self.assertIn('showMobileToast("Long-press field and Paste")', viewer)
        ...
        self.assertIn('id="mobile-help-button"', viewer)
        self.assertIn('aria-label="Open interaction guide"', viewer)
        self.assertIn('id="mobile-guide-overlay"', viewer)
        self.assertIn('id="mobile-guide-sheet"', viewer)
        self.assertIn('role="dialog" aria-modal="true"', viewer)
        self.assertIn('Using the Computer', viewer)
        self.assertIn('Moving Around', viewer)
        self.assertIn('Typing and the Clipboard', viewer)
        self.assertIn('When a Pointer Is Easier', viewer)
        self.assertIn('Slide one finger to move the pointer. Tap to click.', viewer)
        self.assertIn('Tap with two fingers, or press and hold.', viewer)
        self.assertIn('Tap the keyboard button to open your phone keyboard.', viewer)
        self.assertIn('Use the clipboard button to copy to or paste from your phone.', viewer)
        self.assertIn('function setGuideOpen(open)', viewer)
        self.assertIn('guideCloseButton.addEventListener("click"', viewer)
        self.assertIn('if (event.key === "Escape" && !guideOverlay.hidden)', viewer)
        self.assertNotIn('iPhone', viewer)
        self.assertIn('id="mobile-pointer-button"', viewer)
        self.assertIn('id="mobile-pointer-menu"', viewer)
        self.assertIn('id="trackpad-mode-toggle"', viewer)
        self.assertIn('id="recenter-pointer"', viewer)
        self.assertIn('id="mobile-trackpad-surface"', viewer)
        self.assertIn('id="mobile-trackpad-pointer"', viewer)
        self.assertIn('#mobile-trackpad-pointer{position:fixed;z-index:18;', viewer)
        self.assertIn('Trackpad mode', viewer)
        self.assertIn('Recenter pointer', viewer)
        self.assertIn('function updateTrackpadPointerVisual()', viewer)
        self.assertIn('pointerVisual.hidden = !enabled', viewer)
        self.assertNotIn('trackpadPointer = { x: touch.clientX', viewer)
        self.assertIn('function setTrackpadMode(enabled)', viewer)
        self.assertIn('function recenterTrackpadPointer()', viewer)
        self.assertIn('function dispatchTrackpadMouse(type, button = 0, buttons = 0)', viewer)
        self.assertIn('function queueTrackpadMove(deltaX, deltaY)', viewer)
        self.assertIn('function flushTrackpadMove()', viewer)
        self.assertIn('requestAnimationFrame(flushTrackpadMove)', viewer)
        self.assertIn('const TRACKPAD_MIN_GAIN = 0.72;', viewer)
        self.assertIn('const TRACKPAD_MAX_GAIN = 1.15;', viewer)
        self.assertNotIn('const TRACKPAD_SENSITIVITY = 1.45;', viewer)
        self.assertIn('function installDirectTouchGestureGuard()', viewer)
        self.assertIn('function handleDirectDragGesture(event)', viewer)
        self.assertIn('event.detail?.type !== "drag"', viewer)
        self.assertIn('event.stopImmediatePropagation()', viewer)
        self.assertIn('["gesturestart", "gesturemove", "gestureend"]', viewer)
        self.assertIn('installDirectTouchGestureGuard();', viewer)
        self.assertIn('rfb?._handleMouseMove(position.x, position.y)', viewer)
        self.assertIn('rfb?._handleMouseButton(position.x, position.y, mask)', viewer)
        self.assertIn('rfb?._handleWheel({', viewer)
        self.assertNotIn('canvas.dispatchEvent(new MouseEvent(type', viewer)
        self.assertNotIn('canvas.dispatchEvent(new WheelEvent("wheel"', viewer)
        self.assertIn('rfb?._cursor?.move', viewer)
        self.assertIn('trackpadSurface.addEventListener("touchstart"', viewer)
        self.assertIn('trackpadSurface.addEventListener("touchmove"', viewer)
        self.assertIn('trackpadSurface.addEventListener("touchend"', viewer)
