...
- `cua-driver-remote-endpoint` — install on the Agent Screens server; selects the isolated Screen 1 or Screen 2 Unix socket.
- `cua-driver-remote` — common SSH transport installed on the Hermes host.
- `cua-driver-screen1` / `cua-driver-screen2` — fixed-screen entry points for Hermes MCP.
...
Override them through `AGENT_SCREEN_CUA_SOCKET_1`, `AGENT_SCREEN_CUA_SOCKET_2`, `AGENT_SCREEN_CUA_DRIVER`, `AGENT_SCREEN_CUA_USER`, or `AGENT_SCREEN_CUA_HOME` when your deployment differs.
...
## 3. Register both native MCP bridges
...
Enable all discovered tools when prompted, then verify both independent transports:
...
Start a new chat session after changing tool schemas. Direct MCP tools remain available as `cua_screen1` and `cua_screen2`, while built-in `computer_use` uses the selected default wrapper.
...
- Keep both CUA sockets local to the Agent Screens server; transport them only through authenticated SSH.
- Do not disable strict host-key checking.
- Do not expose VNC, CDP, Websockify, or CUA sockets directly to the internet.
- Use separate sockets for Screen 1 and Screen 2 so one bridge cannot silently control the wrong display.
